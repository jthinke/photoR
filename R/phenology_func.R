...
phenology_func<-function(dat, rdat, tabletype="Raw", degf=10){
  ...
  if(tabletype=="Raw"){
    ...
    # update numbers of eggs that must have been in the nest given maximum n of chick and creche obs
    prod.dat<-data.frame(Chick=app_out$N_Hatch, Creche=app_out$N_Creche)
    prod.dat<-apply(prod.dat, 1, max)
    app_out$N_Hatch<-prod.dat
    #write.csv(app_out, file="app_out_raw.csv")
  }
  if(tabletype=="CEMP Protocol A6b"){
    # create A6b data from the output file created by the phenology_func
  ...
  if(tabletype=="CEMP Protocol A9"){
    ...
    #
    #mode: the following code for computing the mode was taken from RBloggers
    Mode = function(x){
      ta = table(x)
      tam = max(ta)
      if (all(ta == tam))
